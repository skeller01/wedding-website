...
- Current maintenance decision: stale external links, remaining dead legacy assets, and gallery curation are the next practical risks/opportunities.
...
### Product Goal
Provide a simple, low-cost, public static wedding archive where visitors can read the wedding story, event memories, historical travel/local context, and an initial static photo gallery without introducing backend complexity.
...
| Couple / Family | Share a polished wedding memory site and browse selected static photos. | User update/prototype absorption |
...
### Solution Summary
Keep the site as plain static HTML/CSS/JS/images on GitHub Pages, preserve the existing pages as a wedding archive, make outbound links non-misleading historical context, maintain the new static gallery page, and clean up unused legacy interaction assets. Treat uploads, tagging, private sharing, backend albums, forms, analytics, a build system, or AWS migration as separate future product decisions.
...
3. Remove or explicitly archive remaining unused countdown and validation assets.
4. Curate and expand the first simple static gallery slice.
...
- Treat the gallery as static HTML/image content unless a later decision changes scope.
...
- The gallery should remain static unless the couple explicitly chooses a dynamic/private photo feature later.
...
- Which additional photos and captions should refine the first static gallery?
